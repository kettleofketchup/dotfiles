    ...
    case $state in
        args)
            ...
            if [[ $lastarg = */* ]]; then
                ...
                # Show usage message
                _message "`just --show $recipe 2>/dev/null`"
...
_just_commands() {
    ...
    # Get recipes with descriptions using JSON dump and jq (if available)
    # Falls back to --summary (no descriptions) if jq not available
    local commands
    if command -v jq &>/dev/null; then
        # Parse JSON to get "name\tdescription" pairs, then escape colons in names
        local raw_output
        raw_output="$(_call_program commands just --dump --dump-format json 2>/dev/null | jq -r '
            def recipes: .recipes // {} | to_entries[] | "\(.value.namepath)\t\(.value.doc // "")";
            def mod_recipes($prefix): .modules // {} | to_entries[] | .value |
                (.recipes // {} | to_entries[] | "\(.value.namepath)\t\(.value.doc // "")"),
                mod_recipes(.key + "::");
            recipes, mod_recipes("")
        ' 2>/dev/null)"
        ...
        if [[ -n "$raw_output" ]]; then
            ...
            while IFS=$'\t' read -r name desc; do
                # Escape colons in name for _describe format (name:description)
                local escaped_name="${name//:/\\:}"
                if [[ -n "$desc" ]]; then
                    commands+=("${escaped_name}:${desc}")
                else
                    commands+=("${escaped_name}")
                fi
            done <<< "$raw_output"
        fi
    fi

    # Fallback to --summary if jq failed or not available
    if [[ -z "$commands" || ${#commands[@]} -eq 0 ]]; then
        commands=(${(s: :)$(_call_program commands just --summary)})
    fi
